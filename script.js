...
function handleCardFlip(cardElement, cardId) {
    if (lockBoard) return;
    if (cardElement.classList.contains("matched") || flippedCards.includes(cardElement)) return;
    ...
    if (flippedCards.length === 2) {
        ...
        if (id1 === id2) {
            matchedCards++;
            document.getElementById("matched-cards").innerHTML = `Matches: ${matchedCards}/8`;
...
function resetBoard() {
    ...
    if (matchedCards === 8) {
        const timerText = document.getElementById("timer").textContent.match(/\d+m \d+s/)[0];
